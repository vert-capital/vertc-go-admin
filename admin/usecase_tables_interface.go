...
type IRepositoryTable interface {
	...
	Get(table Table, id string) (response map[string]interface{}, err error)
	Create(table Table, fields Fields) (response ResponseCreateUpdateDelete, err error)
	Update(table Table, fields Fields, id string) (response ResponseCreateUpdateDelete, err error)
